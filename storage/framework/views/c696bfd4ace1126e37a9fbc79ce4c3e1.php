<?php $__env->startSection('title', 'الخطة الأكاديمية'); ?>
...
<?php if($error): ?>
    <div class="rounded-2xl p-6 mb-6 bg-error/10 border border-error/20">
        <p class="font-semibold text-error"><?php echo e($error); ?></p>
    </div>
<?php elseif(!$profile): ?>
...
<div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
    ...
    <div class="flex items-center gap-2">
        <span class="text-xs text-text-muted"><?php echo e(count($courses ?? [])); ?> مقرر</span>
    </div>
</div>
...
<div class="p-4 rounded-xl border border-border bg-surface shadow-sm mb-6">
    <div class="flex flex-wrap items-center gap-4 text-sm">
        <div class="flex items-center gap-2">
            ...
            <span class="font-bold text-text-primary"><?php echo e(auth()->user()->first_name ?? ''); ?> <?php echo e(auth()->user()->last_name ?? ''); ?></span>
        </div>
        <div class="hidden sm:block h-6 w-px bg-border"></div>
        <div class="flex items-center gap-2">
            <span class="text-text-muted">GPA:</span>
            <span class="font-bold text-text-primary"><?php echo e(number_format($profile['cumulative_gpa'] ?? 0, 2)); ?></span>
        </div>
        <div class="hidden sm:block h-6 w-px bg-border"></div>
        <div class="flex items-center gap-2">
            <span class="text-text-muted">المستوى:</span>
            <span class="font-bold text-text-primary"><?php echo e($profile['level'] ?? '1'); ?></span>
        </div>
        <div class="hidden sm:block h-6 w-px bg-border"></div>
        <div class="flex items-center gap-2">
            <span class="text-text-muted">الساعات:</span>
            <span class="font-bold text-text-primary"><?php echo e($graduationProgress ? $graduationProgress['credits_earned'] : 0); ?>/<?php echo e($graduationProgress ? $graduationProgress['credits_required'] : 0); ?></span>
        </div>
        <div class="hidden sm:block h-6 w-px bg-border"></div>
        <div class="flex items-center gap-2">
            <span class="text-text-muted">تاريخ التخرج:</span>
            <span class="font-bold text-text-primary"><?php echo e($graduationProgress && $graduationProgress['estimated_graduation_date'] ? date('F Y', strtotime($graduationProgress['estimated_graduation_date'])) : 'غير محدد'); ?></span>
        </div>
    </div>
</div>
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4">
        <div class="flex-1 w-full">
            <div class="flex items-center justify-between mb-1.5">
                ...
                <span class="text-xs font-bold text-text-primary"><?php echo e($completed); ?>/<?php echo e($total); ?> مقرر (<?php echo e($pct); ?>%)</span>
            </div>
        ...
        <div class="flex flex-wrap items-center gap-y-2 gap-x-4 text-xs shrink-0">
            <span class="flex items-center gap-1"><span class="status-dot status-dot-completed"></span> <?php echo e($completed); ?> مكتمل</span>
            <span class="flex items-center gap-1"><span class="status-dot status-dot-in-progress"></span> <?php echo e($inProgress); ?> قيد التقدم</span>
            <span class="flex items-center gap-1"><span class="status-dot status-dot-not-started"></span> <?php echo e($notStarted); ?> غير مسجل</span>
            <?php if($failed > 0): ?>
            <span class="flex items-center gap-1"><span class="status-dot status-dot-failed"></span> <?php echo e($failed); ?> راسب</span>
            <?php endif; ?>
        </div>
    </div>
</div>
...
<div class="space-y-6">
    <?php $__empty_1 = true; $__currentLoopData = $groupedCourses ?? []; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $semesterLabel => $semesterCourses): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
    <div class="semester-section p-5 rounded-xl border border-border bg-surface shadow-sm" data-semester="<?php echo e($semesterLabel); ?>">
        <div class="flex items-center justify-between mb-4">
            <h2 class="text-base font-bold text-text-primary"><?php echo e($semesterLabel); ?></h2>
            <span class="text-xs text-text-muted"><?php echo e(count($semesterCourses)); ?> مقررات</span>
        </div>
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            ...
            <div class="course-card p-4 rounded-xl cursor-pointer course-card-base flex flex-col justify-between h-full"
                ...
                data-code="<?php echo e($course['code']); ?>">
                ...
                <div>
                    <div class="flex items-start justify-between gap-2 mb-3">
                        <div class="h-10 w-10 rounded-lg flex items-center justify-center font-bold text-xs bg-surface border border-border text-primary shrink-0">
                            <?php echo e(substr($course['code'], -4)); ?>
                    ...
                    <h3 class="text-sm font-bold mb-0.5 text-text-primary"><?php echo e($course['name']); ?></h3>
                    <p class="text-[11px] mb-3 text-text-secondary"><?php echo e($course['code']); ?> · <?php echo e($course['credit_hours']); ?> ساعة</p>
                </div>
...
<?php echo $__env->make('layouts.dashboard', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH E:\New folder\رفيق الطالب\resources\views/academic/plan.blade.php ENDPATH**/ ?>
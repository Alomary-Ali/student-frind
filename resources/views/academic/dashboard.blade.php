...
@section('title', 'لوحة القيادة الأكاديمية')
@section('content')

@if($error)
    <x-rf-alert variant="critical" title="خطأ">
        {{ $error }}
    </x-rf-alert>
@elseif(!$profile)
    <x-rf-empty-state
        title="لا يوجد ملف أكاديمي"
        description="يرجى التواصل مع الإدارة لإنشاء الملف الأكاديمي"
    />
@else
{{-- Status Strip --}}
    ...
    <div class="flex flex-wrap items-center justify-between gap-4">
        <div class="flex flex-wrap items-center gap-6">
            <div>
                ...
                <p class="text-sm font-black mt-0.5 text-text-primary">{{ $profile['current_semester'] ?? 'غير محدد' }}</p>
            </div>
            ...
            <div>
                ...
                <div class="flex items-center gap-2 mt-0.5">
                    ...
                    <p class="text-sm font-black text-accent">{{ $profile['academic_standing'] ?? 'ممتاز' }}</p>
                </div>
            </div>
            <div class="h-8 w-px bg-border"></div>
            <div>
                <p class="section-label">نسبة التقدم</p>
                <p class="text-sm font-black mt-0.5 text-text-primary">{{ $graduationProgress ? number_format($graduationProgress['completion_percentage'], 0) . '% مكتمل' : '0% مكتمل' }}</p>
            </div>
        </div>
        @if(count($alerts ?? []) > 0)
        <x-rf-badge variant="error" :dot="true">
            تنبيهات: {{ count($alerts) }}
        </x-rf-badge>
        @endif
    </div>
</div>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-2.5 mb-6">
    ...
    <div class="space-y-2.5" data-observe-section="academic-dashboard">
        ...
        <div class="unit-card">
            <div class="flex items-start justify-between mb-3">
                <div>
                    ...
                    <p class="text-[2.25rem] font-black leading-none mt-2 text-text-primary">{{ number_format($profile['cumulative_gpa'] ?? 0, 2) }}</p>
            ...
            <x-rf-progress :value="min(100, ($profile['cumulative_gpa'] ?? 0) / 4 * 100)" variant="accent" size="sm" :showPercentage="false" />
            <p class="text-[10px] font-black mt-1.5 text-accent">{{ min(100, round(($profile['cumulative_gpa'] ?? 0) / 4 * 100)) }}% من الحد الأقصى</p>
        </div>
        ...
        <div class="unit-card">
            <div class="flex items-start justify-between mb-3">
                <div>
                    ...
                    <div class="flex items-baseline gap-1.5 mt-2">
                        <span class="text-[2.25rem] font-black leading-none text-text-primary">{{ $graduationProgress ? $graduationProgress['credits_earned'] : 0 }}</span>
                        <span class="text-base font-semibold text-text-muted">/ {{ $graduationProgress ? $graduationProgress['credits_required'] : 0 }}</span>
                    </div>
            ...
            <x-rf-progress :value="$graduationProgress ? $graduationProgress['completion_percentage'] : 0" variant="primary" size="sm" :showPercentage="false" />
            <p class="text-[10px] font-black mt-1.5 text-primary">{{ $graduationProgress ? number_format($graduationProgress['completion_percentage'], 0) : 0 }}% من متطلبات التخرج</p>
        </div>
        ...
        <div class="unit-card">
            ...
            <p class="text-lg font-black text-text-primary">{{ $profile['academic_status'] ?? 'مستقر' }}</p>
            <p class="text-xs mt-0.5 text-text-secondary">المستوى {{ $profile['level'] ?? '1' }}</p>
        </div>
    </div>

    {{-- Col 2: Quick Actions + Alerts --}}
    <div class="space-y-2.5" data-observe-section="academic-dashboard">
            ...
            <div class="grid grid-cols-2 gap-2.5">
                ...
                <a href="{{ $action['route'] === '#' ? '#' : route($action['route']) }}"
                   class="flex flex-col items-center gap-2 p-4 rounded-xl hover:shadow-md active:scale-95 transition-all group bg-background">
                    ...
                    <span class="text-[11px] font-bold text-center leading-tight text-text-secondary">{{ $action['label'] }}</span>
                </a>
                @endforeach
            </div>
        </div>
        ...
        <div class="unit-card">
            <div class="flex items-center justify-between mb-4">
                ...
                <x-rf-badge variant="error">{{ count($alerts) }}</x-rf-badge>
            </div>
            <div class="space-y-2.5">
                @foreach($alerts as $alert)
                <div class="p-3 rounded-xl border-r-[3px] border-accent bg-accent/5 {{ $alert->severity === 'critical' ? 'alert-critical' : ($alert->severity === 'high' ? 'alert-high' : 'alert-normal') }}">
                    <div class="flex items-center gap-2 mb-1.5">
                        <svg class="h-4 w-4 {{ $alert->severity === 'critical' ? 'text-critical' : ($alert->severity === 'high' ? 'text-high' : 'text-normal') }}" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2" aria-hidden="true">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/>
                        </svg>
                        <span class="text-xs font-bold {{ $alert->severity === 'critical' ? 'text-critical' : ($alert->severity === 'high' ? 'text-high' : 'text-normal') }}">
                            {{ $alert->alertType === 'low_gpa' ? 'تحذير GPA' : ($alert->alertType === 'credit_deficit' ? 'نقص ساعات' : ($alert->alertType === 'graduation_delay' ? 'تأخر تخرج' : 'تنبيه')) }}
                        </span>
                    </div>
                    <p class="text-sm font-bold text-text-primary">{{ $alert->message }}</p>
                    <p class="text-xs mt-0.5 text-text-muted">{{ date('d M Y', strtotime($alert->createdAt)) }}</p>
                </div>
                @endforeach
            </div>
        </div>
            ...
            <div class="relative z-10">
                ...
                <div class="space-y-3.5">
                    <div class="rounded-xl p-3 bg-white/10 border border-white/10">
                        ...
                        <p class="text-white font-black text-sm">{{ $graduationProgress && $graduationProgress['estimated_graduation_date'] ? date('F Y', strtotime($graduationProgress['estimated_graduation_date'])) : 'غير محدد' }}</p>
                    </div>
                    <div class="rounded-xl p-3 bg-white/10 border border-white/10">
    ...
    {{-- Col 3: Needs Attention + Academic Journey --}}
    <div class="space-y-2.5" data-observe-section="academic-dashboard">
        ...
        <div class="unit-card">
            <div class="flex items-center justify-between mb-4">
                ...
                <x-rf-badge variant="error">{{ count($alerts ?? []) }} عناصر</x-rf-badge>
            </div>
            @if($alerts && count($alerts) > 0)
                <div class="grid grid-cols-1 gap-3">
                    @foreach(array_slice($alerts, 0, 3) as $alert)
                        <div class="p-4 rounded-xl border-r-[3px] {{ $alert->severity === 'critical' ? 'alert-critical' : ($alert->severity === 'high' ? 'alert-high' : 'alert-normal') }}">
        ...
        <div class="unit-card">
            <div class="flex items-center justify-between mb-6">
                ...
                <x-rf-badge variant="primary">{{ $graduationProgress ? number_format($graduationProgress['completion_percentage'], 0) . '% مكتمل' : '0% مكتمل' }}</x-rf-badge>
            </div>

            <x-rf-progress :value="$graduationProgress ? $graduationProgress['completion_percentage'] : 0" variant="gradient" size="md" :showPercentage="false" />
                    ...
                    @for($level = 1; $level <= $totalLevels; $level++)
                        <div class="flex flex-col items-center gap-2 min-w-[75px] md:min-w-0 flex-shrink-0">
                            @if($level < $currentLevel)
                                ...
                                <span class="text-[11px] font-bold text-accent">المستوى {{ $level }}</span>
                            ...
                            @elseif($level == $currentLevel)
                                ...
                                <span class="text-[11px] font-black text-primary">المستوى {{ $level }}</span>
                            ...
                            @else
                                ...
                                <span class="text-[11px] font-semibold text-text-muted">المستوى {{ $level }}</span>

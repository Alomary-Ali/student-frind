...
@section('description', 'إنشاء حساب جديد على منصة رفيق الطالب')
...
<form method="POST" action="{{ route('register') }}" class="space-y-5 animate-fade-in-up">
    @csrf
...
<p class="text-center text-sm text-text-secondary mt-6">
    لديك حساب بالفعل؟
    <a href="{{ route('login') }}" class="font-bold text-primary hover:text-accent transition-colors">تسجيل الدخول</a>
</p>
